...
struct ArcNode {
    int adjvex; // 邻接顶点的编号
    ArcNode *next; // 指向下一个邻接顶点的指针
};
// 顶点结点
struct VNode {
int data; //顶点信息 
ArcNode *firstarc; // 指向第一个邻接顶点的指针 //指向vi的第一个邻接顶点（边）
};
// 图的邻接表表示
...
int main() {
    int arr[] = {0, 12, 11, 13, 5, 6, 7}; // 注意：数组下标从1开始，所以第0个位置是无效的
    int n = 6;
...
>    - 如果图不连通，返回 -1 并打印提示信息。 ==时间复杂度：O(elog2e)， 适合用于边稀疏图== elonge
